    ...
    with get_session() as session:
        ...
        if runner == "local":
            ...
            if runner_invoke_url:
                result = invoke_function_via_curl(runner_invoke_url, agents, thread_id, auth, "", params)
            else:
            ...
            result = invoke_function_via_lambda(function_name, agents, thread_id, auth, "", params)
        ...
        response = OpenAIRun(
            ...
            instructions=run_model.instructions or "",
            tools=[],
            metadata={},
            usage=result.get("usage"),
            temperature=run_model.temperature or 1.0,
            top_p=run_model.top_p or 1.0,
            object="thread.run",
...
async def get_run(
    ...
    with get_session() as session:
        ...
        return OpenAIRun(
            ...
            assistant_id=run_model.assistant_id,
            thread_id=run_model.thread_id,
            status=run_model.status,
            started_at=run_model.started_at,
            expires_at=run_model.expires_at,
            cancelled_at=run_model.cancelled_at,
            failed_at=run_model.failed_at,
            completed_at=run_model.completed_at,
            last_error=run_model.last_error,
            model=run_model.model,
            instructions=run_model.instructions or "",
            tools=run_model.tools or [],
            metadata=run_model.meta_data or {},
            usage=run_model.usage,
            temperature=run_model.temperature or 1.0,
            top_p=run_model.top_p or 1.0,
            object="thread.run",
            incomplete_details=None,
